...
return {
        ...
        require('xcodebuild').setup {
            ...
            console_logs = {
                ...
                filter_line = function(line)
                    if line:find 'Metal API Validation' then
                        return false
                    end
                    if line:find 'libMobileGestalt' then
                        return false
                    end
                    if line:find 'UICollectionViewFlowLayout' then
                        return false
                    end
                    if line:find 'nw_connection_' then
                        return false
                    end
                    if line:find 'SecTaskLoadEntitlements' then
                        return false
                    end
                ...
                format_line = function(line)
                    if line:find '%[ERROR%]' or line:find 'error:' then
                        return line, 'DiagnosticError'
                    end
                    if line:find '%[WARNING%]' or line:find 'warning:' then
                        return line, 'DiagnosticWarn'
                    end
                    if line:find '%[DEBUG%]' then
                        return line, 'DiagnosticHint'
                    end
                    return line
                end,
            },
    ...
    vim.keymap.set("n", "<leader>xx", "<cmd>XcodebuildQuickfixLine<cr>", { desc = "Quickfi[x] Line" })
    vim.keymap.set("n", "<leader>xa", "<cmd>XcodebuildCodeActions<cr>", { desc = "Show Code [A]ctions" })
    vim.keymap.set("n", "<leader>xg", "<cmd>!pushd . && xcodegen; popd<cr>",
      { desc = "Run Xcode[g]en" })
    end,
}
